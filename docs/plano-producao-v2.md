...
| 12.5 | Entrar com Google   | IdP social no mesmo pool (`identity_provider=Google` no `/oauth2/authorize`); vinculação de conta na trigger `PreSignUp_ExternalProvider` preserva `users.id = sub` — sem migração. Risco residual aceito no threat model; flag `google_enabled` liga staging antes de prod. Spec: [DF-17](../specs/drafts/df17-login-google.md).                                                                                               |
