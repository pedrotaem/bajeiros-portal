...
| ~~Equipes e convites (fase 14)~~ **FEITO — PR #11** (base #10)                                                                                                               | Pedro  | mergear após #9/#10. Migração 0002 (fns SECURITY DEFINER p/ aceite + perfis; policy UPDATE team_members), policy layer `can()`/`outranks()` em `src/policy.ts`, convite por link copiável (token 1× na resposta, hash no banco, 7d, vinculado ao e-mail, sem enumeração C9), transferência de projeto p/ equipe, painel Equipes no web, 19 testes por papel (gate M2), smoke Chrome ok |
| **PRÓXIMO BLOCO:** quick wins UX do estudo (Esc fecha modais, z-index labels 3D, câmera fit, toast sessão expirada) → inversão checklist → onboarding; purge job 30d (LGPD); Cognito real (bloqueado em AWS) | Claude | ordem combinada                                                                                                                                                                                                                                                                                                                                                                                |
