...
- **FR-1.7** (DF-29) Alternador **"Rótulos"** na barra do viewport, ligado por padrão, esconde e mostra os rótulos de id dos nós. Desligado, só o nó selecionado (e o primeiro nó capturado em "adicionar membro") continua rotulado. É estado de sessão: não vai para o JSON e não muda ao importar ou restaurar o template. Rótulos de ancoragem, volante e plano já são condicionais à seleção e não são afetados.
...
- **FR-13.5** (DF-22 §11) **Cota flutuante:** selecionar um membro abre um campo numérico (`drei/Html`, variante `measure`) acima do ponto médio do tubo, com o comprimento atual, foco e texto selecionados. Mesmo contrato do campo do inspetor: Enter aplica (`setDistance(a, b, alvo, 'b')`), Esc descarta, sair aplica. Ponta `b` travada → campo desabilitado com o cadeado e o nome do nó. O campo captura ponteiro e teclado para não contar como "clique fora"; some em "adicionar membro". As duas entradas (flutuante e inspetor) fazem a mesma ação.
...
### US-17 — Módulo de suspensão (DF-30 v1, implementada)

Como projetista, escolho o tipo de suspensão de cada eixo, fixo o entre-eixos e vejo roda, bandeja e amortecedor na cena, para que a gaiola feche com o carro — não só com o regulamento.

- **FR-17.1** `Cage.suspension?: SuspensionConfig` — `wheelbaseMm` e, por eixo, `{ type, wheelCenter (lado L), tire: { od, width, rim } }`. **Opt-in**: ausente, o projeto se comporta como em US-4 (ancoragens soltas + SUSP.1). O template nasce com duplo A na dianteira, braço arrastado na traseira (16 ancoragens), pneu 22×7-10 e entre-eixos 1121 mm.
- **FR-17.2** Tipos por eixo: dianteira `duplo-a | mcpherson`; traseira `duplo-a | semi-trailing | trailing` — ids idênticos ao catálogo da ficha (DF-21 §5.4). Cada tipo define os **papéis de ancoragem por roda** (`ANCHOR_ROLES_BY_TYPE`): duplo A `sup1 sup2 inf1 inf2 amort`; os demais `inf1 inf2 amort`.
- **FR-17.3** "Configurar suspensão" infere o tipo das ancoragens presentes (`sup1`+`sup2` → duplo A; senão McPherson/braço arrastado), põe o centro de roda fora da ancoragem mais externa (`|x| máx + 250`) na altura e z médios das bandejas, e declara o entre-eixos igual ao medido.
- **FR-17.4** Trocar o tipo **reconcilia** as ancoragens do eixo: mantém as com papel no tipo novo, remove as sem papel (limpando travas e seleção, FR-15.5), cria as que faltam a partir das vizinhas (`sup_i = inf_i + 200 mm`, `inf_i = sup_i − 200 mm`, `amort` 250 mm acima do meio das bandejas), sempre em par L/R espelhado. Tipo não permitido para o eixo é ignorado.
- **FR-17.5** Centro de roda: um ponto por eixo guardado para o lado L, espelhado para o R; **cubo** na cor de ancoragem (mesma cor, outra forma — design-system §9.3), arrastável e editável por X/Y/Z; travável pelo id do eixo (`roda-dianteira`, `roda-traseira`) — travar o centro trava os dois lados. Bitola do eixo = `2·|x|`; entre-eixos medido = `|z(traseira) − z(dianteira)|`.
- **FR-17.6** `SUSP.2` (cond.): `|medido − declarado| ≤ 5 mm` → `pass`; senão `fail` com medido, declarado e diferença. `SUSP.3` (cond.): cada eixo tem, dos dois lados, exatamente os papéis do tipo — faltando → `fail` com `presence: true`; sobrando → `fail` listando o que remover. `SUSP.1` segue igual, sobre o conjunto reconciliado.
- **FR-17.7** Corpos genéricos derivados (`suspensionBodies`, nunca gravados): bandejas/braços (r 11), manga (rótulas a ±100 mm do centro, 40 mm para dentro da face interna do pneu), cubo e amortecedor (corpo r 16 + haste; coluna McPherson r 20) como **tubos sólidos** com o material dos membros na cor `--bj-3d-suspension` (um passo acima do secundário); roda com volume — carcaça revolucionada em `--bj-3d-rubber`, garras off-road alternadas, letra branca na lateral externa, aro em prato na cor do membro primário. O cilindro `tire` de `suspensionBodies` é o envelope usado pelo enquadramento e pelos testes. `raycast` nulo em tudo. Ancoragem exigida ausente derruba só o corpo que dependia dela.
- **FR-17.8** Barra do viewport: alternador **"Suspensão"** (corpos; desabilitado sem configuração) e **"Ancoragens"** (marcadores de ancoragem **e** centros de roda). Ligados por padrão. O enquadramento das vistas (FR-16.2) inclui as rodas quando os corpos estão visíveis. Legenda: "ancoragem · suspensão".
- **FR-17.9** Aba **Suspensão** no inspetor: entre-eixos declarado (Enter aplica) com medido e estado, um bloco por eixo (tipo, centro de roda, trava, bitola medida, pneu), a **lista de ancoragens** (que sai da aba Seleção) e "Remover configuração" — que apaga só `suspension` e as travas dos centros; as ancoragens ficam.
- **FR-17.10** Importar saneia (`sanitizeSuspension`): número não finito, pneu não positivo ou tipo inválido para o eixo descartam a configuração inteira; centro com `x > 0` é normalizado para o lado L; a trava órfã do centro cai com ela.
- **FR-17.11** Ficha (DF-21): `suggestFrom` passa a sugerir `dim.entre-eixos`, `dim.bitola-dianteira`, `dim.bitola-traseira`, `susp.tipo-dianteiro` e `susp.tipo-traseiro` — só com `suspension` presente; os campos seguem editáveis à mão.

### US-18 — Recalcular: re-identificar pontos denominados e reavaliar (DF-31, implementada)

Como projetista que modela livre, quero que o nó que eu criei no encontro dos tubos certos passe a valer como o ponto do regulamento, sem renomear à mão — e que o checklist reavalie tudo num clique.

- **FR-18.1** Uma letra do regulamento é um **encontro de tipos de membro** (`LETTER_RULES`, `model/identify.ts`), em ordem de prioridade: A = RRH + (ALC | LFS | FAB_LOW | LFDB); B = RRH + (BLC | RHO | FAB_UP); H = RRH + SHC; S = RRH + (SIM | FAB_MID); C = RHO + (CLC | FBM_UP); D = FBM_UP + (FBM_LOW | DLC); F = LFS + (FLC | FBM_LOW); I = LFS + ILC; R = RLC + FAB_*; P = FAB_UP + (FAB_LOW | SIM) sem RRH nem RLC. O lado vem do sinal de `x`.
- **FR-18.2** `identifyNamedPoints` propõe id de regulamento **só para nós genéricos** (sem letra do catálogo) com letra inequívoca; nunca toca em nó já lettered. Recusa, com razão relatada: nó no plano de simetria (sem lado), vaga já ocupada, dois candidatos para a mesma vaga.
- **FR-18.3** `applyRenames` renomeia nós, membros, continuidade, travas e `namedExtra` (letra sai da lista extra). Ancoragens, volante e suspensão guardam posição, não id: intactos.
- **FR-18.4** Ação `recalculate()`: aplica a identificação, saneia como a importação (continuidade, travas, `namedExtra`, suspensão, seções), reconcilia as ancoragens com o tipo de cada eixo quando há `suspension`, cancela "adicionar membro" pendente, segue a seleção pelo rename e grava um relatório.
...
| AC-20 | DF-29: alternador "Rótulos" nasce ligado, alterna, e importar/restaurar não o muda; o estado não entra no JSON                                                                                                                                                                                                                              | vitest ✔ (`scene-store.test.ts`) · manual/browser ✔                                                     |
