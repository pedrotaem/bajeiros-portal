...
- **FR-17.1** `Cage.suspension?: SuspensionConfig` — `wheelbaseMm` e, por eixo, `{ type, wheelCenter (lado L), tire: { od, width, rim } }`. **Opt-in**: ausente, o projeto se comporta como em US-4 (ancoragens soltas + SUSP.1). O template nasce com duplo A nos dois eixos, pneu 22×7-10 e entre-eixos 1102 mm.
...
| AC-22 | DF-30: template passa SUSP.2/SUSP.3; entre-eixos 1400 falha com −298; McPherson na dianteira deixa 16 ancoragens e limpa travas; voltar a duplo A recria as superiores espelhadas; centro R em +600 põe o L em −600; JSON com tipo inválido importa sem módulo; ficha sugere 1102/1106/1258 e `duplo-a` | vitest ✔ (`suspension.test.ts`, `b6-suspension.test.ts`, `suspension-store.test.ts`, `suggest.test.ts`) |
