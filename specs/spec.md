...
- **FR-17.7** Corpos genéricos derivados (`suspensionBodies`, nunca gravados): cilindros para pneu (r `od/2` × `width`), aro, cubo, manga (rótulas a ±100 mm do centro, 40 mm para dentro da face interna do pneu), bandejas/braços (r 11) e amortecedor (r 16; coluna McPherson r 20). `raycast` nulo, cor `--bj-3d-anchor-ok` translúcida com arame opaco (§9.5). Ancoragem exigida ausente derruba só o corpo que dependia dela.
