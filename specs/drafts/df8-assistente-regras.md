...
- **Status:** ✅ **IMPLEMENTADA** (2026-08-23, `cb1098a` / PR #12; ligada ao AI Gateway por
  SigV4 em 2026-08-26, `1437377` / PR #27). O gateway é stack de outro repo
  (`bajeiros-ai-gateway`) e entra por `terraform_remote_state` em `infra/envs/{staging,prod}` —
  **aplicar o gateway antes do portal**. Não vai para `spec.md`, que é do validador. Detalhe
  original: revisão 3 personas em `docs/revisao-assistente-ia.md` (C1–C19 aplicados). Implementado: módulo `assistant` no `apps/api` (proxy SSE, gate de aviso via `audit_events` `assistant.notice_accept`, quota 20/dia contada em `assistant_log`, rateKey HMAC sal diário), `AssistantPanel.tsx` (chat streaming, aviso de transparência, quota, mini-renderer), ação "perguntar ao assistente" no checklist, registro em `assistant_log` (DF-9). Desvios da spec: em dev o proxy roda no próprio Hono/Node (SSE ok) — a rota Function URL (C1/C2) só é necessária no deploy AWS; aceite do aviso na trilha `audit_events` (não em `consents` — coerente com C9); quota derivada de `assistant_log` (sem tabela `assistant_usage`; contagem não-atômica aceita em dev); citações inline no texto (gateway v0 via Agent SDK/subscription — evento `citation` estruturado volta na fase API/G1); persistência de conversas EXISTE via `assistant_log` (decisão DF-9, aviso declara). Atualização (decisão do usuário, 2026-08-23): **anônimo pode usar com 2 perguntas/dia** (funil de conta) — quota por IP em memória no servidor (best-effort) + auth opcional na rota; anônimo NÃO é persistido em `assistant_log` (promessa "anônimos não rastreados") e o aceite do aviso fica só no browser; sobrepõe o requisito de e-mail verificado (C16) p/ a degustação — o teto de custo real segue sendo a quota global do gateway.
