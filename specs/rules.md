...
| SUSP.2      | auto (portal) · cond. (DF-30) | Entre-eixos declarado no módulo de suspensão × medido entre os centros de roda                                       | ± 5 mm                                 | só com `Cage.suspension`; `fail` traz medido, declarado e diferença                                              |
| SUSP.3      | auto (portal) · cond. (DF-30) | Cada eixo tem, dos dois lados, exatamente os papéis de ancoragem do tipo escolhido (duplo A 5; McPherson e braços 3) | conjunto exato                         | faltando → `presence: true`; sobrando → lista o que remover; trocar o tipo na aba Suspensão reconcilia           |
