...
interface State {
  ...
  recalculate: () => void
  // DF-32 — desfazer/refazer. Só a GAIOLA entra no histórico: seleção, alternadores e câmera
  // são estado de tela e voltam sozinhos ao estado que fizer sentido para a gaiola restaurada.
  past: Cage[]
  future: Cage[]
  /** assinatura da última mudança gravada (chaves/nós tocados) — para fundir digitação */
  histSig: string | null
  histAt: number
  /** 0 = fora de gesto · 1 = gesto aberto, nada gravado ainda · 2 = gesto aberto, já gravou */
  histGesture: 0 | 1 | 2
  undo: () => void
  redo: () => void
  /** Arrasto no 3D: um gesto inteiro é um passo só de desfazer, do pointerdown ao pointerup. */
  beginGesture: () => void
  endGesture: () => void
...
/** Teto do histórico: 100 gaiolas de ~15 KB são desprezíveis, e ninguém desfaz mais que isso. */
const HISTORY_MAX = 100
/** Mudanças com a MESMA assinatura dentro desta janela fundem num passo: é digitação num campo. */
const COALESCE_MS = 800

/**
 * O que a mudança tocou: chaves de primeiro nível da gaiola, e o id de cada nó movido.
 * `withMirror` recria `nodes` a cada passo mas preserva a referência dos nós parados, então a
 * comparação por referência separa "moveu AL (e o espelho AR)" de "moveu BL".
 */
function changeSignature(a: Cage, b: Cage): string {
  const keys = new Set<string>()
  for (const k of new Set([...Object.keys(a), ...Object.keys(b)])) {
    if (k === 'nodes') {
      for (const id of new Set([...Object.keys(a.nodes ?? {}), ...Object.keys(b.nodes ?? {})])) {
        if (a.nodes?.[id] !== b.nodes?.[id]) keys.add(`nodes:${id}`)
      }
    } else if (
...
/** Seleção que ainda existe na gaiola restaurada — o resto vira null, sem fantasma. */
function selectionFor(cage: Cage, s: State): Partial<State> {
  return {
    selectedNode: s.selectedNode && cage.nodes[s.selectedNode] ? s.selectedNode : null,
    selectedMember:
      s.selectedMember && cage.members.some((m) => m.id === s.selectedMember)
        ? s.selectedMember
        : null,
    selectedAnchor:
      s.selectedAnchor && (cage.anchors ?? []).some((a) => a.id === s.selectedAnchor)
        ? s.selectedAnchor
        : null,
    selectedSteering:
      s.selectedSteering && (cage.steering?.points ?? []).some((p) => p.id === s.selectedSteering)
        ? s.selectedSteering
        : null,
    selectedWheel: cage.suspension ? s.selectedWheel : null,
    selectedPlane: null,
    pending: null,
  }
}
...
export const useStore = create<State>((rawSet, get) => {
  // Todo `set` das ações passa por aqui: se a gaiola mudou, o estado anterior vai para `past`.
  // Desfazer/refazer usam `rawSet` e não entram no próprio histórico.
  const set: StoreApi<State>['setState'] = (partial, replace) => {
    const before = get()
    ;(rawSet as (p: typeof partial, r?: boolean) => void)(partial, replace)
    const after = get()
    if (after.cage === before.cage) return
    const now = Date.now()
    if (before.histGesture === 2) return
    const sig = changeSignature(before.cage, after.cage)
    if (before.histGesture === 1) {
      rawSet({
        past: [...before.past, before.cage].slice(-HISTORY_MAX),
        future: [],
        histGesture: 2,
        histSig: sig,
        histAt: now,
    ...
    if (sig === before.histSig && now - before.histAt < COALESCE_MS) {
      // digitação: o passo anterior já guarda a gaiola de antes do primeiro caractere
      rawSet({ future: [], histAt: now })
      return
    }
    rawSet({
      past: [...before.past, before.cage].slice(-HISTORY_MAX),
  ...
  return {
    ...
    undo: () => {
      ...
      rawSet({
        ...
        future: [s.cage, ...s.future].slice(0, HISTORY_MAX),
        histSig: null,
        histGesture: 0,
        ...selectionFor(prev, s),
      })
    ...
    redo: () => {
      ...
      rawSet({
        cage: next,
        past: [...s.past, s.cage].slice(-HISTORY_MAX),
    ...
    beginGesture: () => rawSet({ histGesture: 1 }),
    endGesture: () => rawSet({ histGesture: 0, histSig: null }),
    ...
    pickNode: (id) =>
      set((s) => {
        ...
        const exists = s.cage.members.some(
          (m) =>
            (m.a === s.pending!.first && m.b === id) || (m.b === s.pending!.first && m.a === id),
        )
