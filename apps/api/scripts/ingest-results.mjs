...
//
// Em staging/produção o banco é Aurora atrás da RDS Data API. O script usa o MESMO driver
// da API (`src/db`), que escolhe por `DB_MODE`:
//
...
try {
  // `withUser` abre a transação e fixa `app.user_id` — a escrita passa pela RLS como o
  // resto do portal (policies *_admin) — e commita no retorno normal, nos dois drivers.
